...
scanf("%s",filename);
printf(" DATA WILL BE WRITTEN TO A FILE CALLED %s\n", filename);
...
// Calculate the complex SPP index at the current wavelength for metal 2
  ...
  // get real part of N_SPP1 and store it to eta1
  eta1 = creal(N_SPP1);

  // get imaginary part of N_SPP1 and store it to kappa1
  kappa1 = cimag(N_SPP1);

 // get real part of N_SPP2 and store it to eta2
  eta2 = creal(N_SPP2);

  // get imaginary part of N_SPP2 and store it to kappa2
  kappa2 = cimag(N_SPP2);
